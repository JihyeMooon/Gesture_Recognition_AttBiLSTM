...
def calculate_precision(outputs, targets):
    ...
    target_view=targets.view(-1).cpu().numpy()
    pred_view=pred.view(-1).cpu().numpy()
    #return  precision_score(targets.view(-1).numpy(), pred.view(-1).numpy(), average = 'macro')
    return  precision_score(target_view, pred_view, average = 'macro')
...
def calculate_recall(outputs, targets):
    ...
    return  recall_score(target_view, pred_view, average = 'macro')